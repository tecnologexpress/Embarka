...
import type {
  IJanelaDeColetaFornecedorDto,
  IAtualizarJanelaDeColetaFornecedorDto,
...
type ModalEditarProps = {
  ...
  item: IJanelaDeColetaFornecedorDto;
};

export const ModalEditarJanelaDeColeta: React.FC<ModalEditarProps> = ({
  abrir,
  aoFechar,
  diasEmUso,
  item,
  ...
  const [formulario, setFormulario] =
    useState<IJanelaDeColetaFornecedorDto>(item);
  const [enviando, setEnviando] = useState(false);

  // Sincroniza o estado do formulário com a propriedade 'item'
  useEffect(() => {
    if (item) {
      setFormulario(item);
    }
  ...
  const DESATIVAR_DIAS = React.useMemo(() => {
    const set = new Set(diasEmUso);
    if (item?.ds_dia_da_semana) {
      set.delete(item.ds_dia_da_semana as TDiasDaSemana); // Permite re-selecionar o dia atual do item
    }
    return set;
  }, [diasEmUso, item]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload: IAtualizarJanelaDeColetaFornecedorDto = {
        id_janela_de_coleta_fornecedor:
          formulario.id_janela_de_coleta_fornecedor,
        ...
        formulario.hr_horario_intervalo_fim
          ? {
              hr_horario_intervalo_inicio:
                formulario.hr_horario_intervalo_inicio,
              hr_horario_intervalo_fim: formulario.hr_horario_intervalo_fim,
            }
          : {}),
      };

      await atualizarJanelaDeColetaFornecedor(payload);
  ...
  return (
    <ModalBase
      abrir={abrir}
      aoFechar={aoFechar}
      titulo="Editar janela de coleta"
    >
      <form
